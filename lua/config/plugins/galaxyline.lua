return {
  ...
  config = function()
    ...
    local Se7enVim = {
      ...
      statusline = {
        path_enabled = true,
        path = 'absolute', --  NOTE: absolute/relative
      },
    }

    local bgcolor = function()
      if Se7enVim.colorscheme == 'nightfly' then
        return '#011627'
      else
        return nil
      end
    ...
    if Se7enVim.statusline.path_enabled then
      table.insert(gls.right, {
        FileName = {
          provider = function()
            ...
            if Se7enVim.statusline.path == 'relative' then
